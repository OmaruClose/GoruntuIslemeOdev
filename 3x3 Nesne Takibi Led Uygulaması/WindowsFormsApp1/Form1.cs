...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        void Finalvideo_NewFrame(object sender, NewFrameEventArgs eventArgs)
        {
            ...
            Cfilter.Green = new IntRange(0, 75);
            Cfilter.Blue = new IntRange(0, 75);
            // apply the filter
            ...
            if (radioButton2.Checked)
            {
                ...
                ColorFiltering Cfilter = new ColorFiltering();
                // set color ranges to keep
                Cfilter.Red = new IntRange(0, 75);
                Cfilter.Green = new IntRange(170, 255);
                Cfilter.Blue = new IntRange(0, 75);
                // apply the filter
                Cfilter.ApplyInPlace(sonimage);
            }
            if (radioButton3.Checked)
            {
                ...
                Cfilter.Red = new IntRange(0, 75);
                Cfilter.Green = new IntRange(0, 75);
                Cfilter.Blue = new IntRange(170, 255);
                // apply the filter
                Cfilter.ApplyInPlace(sonimage);
            }
            ...
            foreach (Rectangle rect in rects)
            {
                ...
                if (rects.Length > 1)
                {
                    ...
                    else if (objectX > 150 && objectX <= 300 && objectY > 240 && objectY <= 360)
                    {
                        serialPort1.Write("8");
                    }
